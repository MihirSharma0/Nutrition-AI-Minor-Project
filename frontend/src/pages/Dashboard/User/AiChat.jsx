...
const AiChat = () => {
    ...
    return (
        <div className="flex flex-col h-[75vh] bg-white/5 backdrop-blur-xl rounded-[2rem] sm:rounded-[2.5rem] border border-white/10 p-4 sm:p-6 lg:p-8 shadow-2xl">
            ...
            <div className="flex-1 overflow-y-auto space-y-4 mb-4 pr-2 custom-scrollbar">
                ...
                {messages.map((msg, idx) => (
                    <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[88%] sm:max-w-[75%] p-3 sm:p-4 rounded-2xl ${msg.role === 'user' ? 'bg-[#a5d391] text-black font-medium rounded-br-none shadow-lg' : 'bg-[#080b12]/80 border border-white/10 text-white/90 rounded-bl-none shadow-md'}`}>
                            {msg.content}
                        </div>
                    </div>
                ))}
            ...
            <form onSubmit={sendMessage} className="flex gap-2 sm:gap-3">
                <input
                    ...
                    className="flex-1 min-w-0 bg-[#080b12]/80 border border-white/10 rounded-2xl p-3 sm:p-4 text-white placeholder:text-white/40 outline-none focus:border-[#a5d391]/50 transition-colors"
                />
                <button type="submit" disabled={loading || !input.trim()} className="shrink-0 bg-[#a5d391] hover:bg-white disabled:opacity-50 text-black px-4 sm:px-6 py-3 sm:py-4 rounded-2xl font-bold transition-all shadow-[0_0_20px_rgba(165,211,145,0.3)] cursor-pointer">
                    Send
                </button>
            </form>
        </div>
